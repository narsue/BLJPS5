...
#else
SubgoalGraph::SubgoalGraph(std::vector<bool> &bits, int width, int height, const char *filename, int memoryLimit, int timeLimit) :PathFindingAlgorithm("SubGoal",AT_SUBGOAL)
#endif
{
	InitializeValues();
#ifdef SG_RUNNING_IN_HOG
...
void SubgoalGraph::findSolution(int sX, int sY, int _eX, int _eY, std::vector<xyLoc> & thePath)
{
	SubgoalAStarSearch(xyLoc(sX, sY), xyLoc(_eX, _eY), INFINITE_COST, thePath);
	//std::reverse(thePath.begin(), thePath.end());
}
SubgoalGraph::SubgoalGraph(const char *filename) :PathFindingAlgorithm("SubGoal", AT_SUBGOAL)
{
	InitializeValues();
	LoadGraph(filename);
...
void SubgoalGraph::AddToOpen(subgoalId sg, cost fVal)
{
	...
	{
		theHeap[0] = (heapElement3(sg, fVal));	// .. replace it
	...
	{	// add element as usual
		theHeap.push_back(heapElement3(sg, fVal));
...
#endif
heapElement3 SubgoalGraph::GetMin()
{
...
{
	heapElement3 elem = theHeap[index];
	int parent;
	parent = (index - 1) >> 1;
...
cost SubgoalGraph::SubgoalAStarSearch(subgoalId & start, subgoalId & goal, cost searchLimit, std::vector<subgoalId> & abstractPath)
{
	...
	int searchCount = 0;
	int expandCount = theHeap.size();
	//FILE* fp = 0;
	////if (finalized)
	//	fp = fopen("SUBGOAL_debug_connections.txt", "w");

#ifdef USE_STACK
	//heapElement3* bestElement = &theHeap[0];
	cost currFCost;
...
#endif
	{
		...
		//	fprintf(fp, "\n\n%d[%d]:(%d,%d)", searchCount, sg, location[sg].x, location[sg].y);
			...
			for (unsigned int i = 0; i < nSuccessors; i++)
			{
				...
				if (generated[succ] != search)
				{
					...
					if (gCost[succ] + HCost(succ, goal) == currFCost)
						theStack.push_back(heapElement3(succ, gCost[succ] + HCost(succ, goal)));
					else
					...
					//	fprintf(fp, "\n\t:%d[%d]: (%d,%d)", expandCount, succ,location[succ].x, location[succ].y);
	...
	//	fclose(fp);
	//if (finalized)
	//	printf("Expansions %d %d\n", searchCount, expandCount);
	...
	if (gCost[goal] != searchLimit)	// Follow the parent pointers to extract the path
	{
		...
		{
		if (!hasPoint(Coordinate(x, y), checkedSol))
		fprintf(fp, ".");
		else
		fprintf(fp, "x");
		}
