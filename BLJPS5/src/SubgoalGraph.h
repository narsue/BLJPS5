...
static std::vector<subgoalId> defaultAbstractPath_2;	// For default parameters
static std::vector<xyLoc> defaultXYPath_2;
static std::vector<mapLoc> defaultPath_2;	

class SubgoalGraph : public PathFindingAlgorithm
{
public:
	//SubgoalGraph();
	...
	bool IsHReachable		// Check if two locations are h-reachable, save the path to the path
		(xyLoc & from, xyLoc & to, std::vector<mapLoc> & path = defaultPath_2, bool append = false);
	...
	cost SubgoalAStarSearch	// Do a search over the subgoal graph, between two subgoals
		(subgoalId & start, subgoalId & goal, cost limit = INFINITE_COST, std::vector<subgoalId> & abstractPath = defaultAbstractPath_2);
	...
	cost SubgoalAStarSearch	// Either use search or lookup, convert abstract path to low level path
		(xyLoc & start, xyLoc & goal, cost limit = INFINITE_COST, std::vector<xyLoc> & thePath = defaultXYPath_2);
	cost GetPath(xyLoc & start, xyLoc & goal, std::vector<xyLoc> & thePath = defaultXYPath_2);
	...
	{
		#ifdef SG_STATISTICS
		std::cout<<"The heap capacity is: "<<theHeap.capacity()<<", which is about "<<(theHeap.capacity()*sizeof(heapElement3))/1024<<"kb."<<std::endl;
		#endif
	}
	...
	void findSolution(int sX, int sY, int _eX, int _eY, std::vector<xyLoc> & thePath);
...
private:
	...
	bool canReplaceTop;
#endif
	std::vector<heapElement3> theHeap;
	std::vector<heapElement3> theStack;
